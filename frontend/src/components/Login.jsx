...
const Login = ({ setActiveSection, onLoginSuccess }) => {
  ...
  useEffect(() => {
    ...
    if (storedAuth?.username) {
      ...
      setRemember(storedAuth.storage === "local");
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      saveAuth({ token: data.token, username: trimmedUsername }, remember);
  ...
  return (
    <div className="bg-[#ffff] p-4 md:px-4 md:py-2 rounded-md shadow relative w-full max-w-md">
      <div className="flex items-center justify-between">
        <h2 className="text-3xl lg:text-3xl font-bold md:mt-2 mb-4 text-[#127487]">🔐Login</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <label htmlFor="remember" className="text-[#2A7B9B] font-semibold">
            Remember me
          </label>
        </div>
